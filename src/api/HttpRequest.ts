...
class HttpRequest {
  ...
  private request<T>(method: HttpMethod, url: string, body: any = null, headers: CustomHeaders = {}): Promise<T> {
  ...
  get<T>(url: string, params: Params = {}, headers: CustomHeaders = {}): Promise<T> {
  ...
  post<T>(url: string, body: any, headers: CustomHeaders = {}): Promise<T> {
    return this.request<T>('POST', url, body, headers);
  }

  // Метод для PUT-запросов
  put<T>(url: string, body: any, headers: CustomHeaders = {}): Promise<T> {
    return this.request<T>('PUT', url, body, headers);
  }

  // Метод для DELETE-запросов
  delete<T>(url: string, body: any = null, headers: CustomHeaders = {}): Promise<T> {
